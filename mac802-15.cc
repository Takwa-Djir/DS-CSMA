...
#include "mac-802_15_4.h"
#include "cmu-trace.h"

// Added by Sushmita to support event tracing
#include "agent.h"
#include "basetrace.h"
#include "wsnrbc/wsnrbc.h"




inline void
Mac802_15_4::checkBackoffTimer()
{
...
inline void
Mac802_15_4::transmit(Packet *p, double timeout)
{
	tx_active_ = 1;
...
inline void Mac802_15_4::setRxState(MacState newState)
{
	rx_state_ = newState;
	checkBackoffTimer();
}

inline void Mac802_15_4::setTxState(MacState newState)
{
	tx_state_ = newState;
	checkBackoffTimer();
}
...
PHY_MIB::PHY_MIB(Mac802_15_4 *parent)
{
	/*
	 * Bind the phy mib objects.  Note that these will be bound
	 * to Mac/802_15_4 variables
	 */
...
MAC_MIB::MAC_MIB(Mac802_15_4 *parent)
{
	/*
	 * Bind the phy mib objects.  Note that these will be bound
	 * to Mac/802_15_4 variables
...
Mac802_15_4::Mac802_15_4() : 
	...
	mhDefer_(this), mhBackoff_(this), mhDS-CSMA_Backoff_(this), mhBeacon_(this), mhProbe_(this)
{
...
/************************************************/
	...
	tcl.evalf("Mac/802_15_4 set basicRate_");
	if (strcmp(tcl.result(), "0") != 0) 
		bind_bw("basicRate_", &basicRate_);
	else
		basicRate_ = bandwidth_;

	tcl.evalf("Mac/802_15_4 set dataRate_");
	if (strcmp(tcl.result(), "0") != 0) 
...
// Added by Sushmita to support event tracing
void Mac802_15_4::trace_event(char *eventtype, Packet *p) 
{
        if (et_ == NULL) return;
        char *wrk = et_->buffer();
	...
	//       Address::instance().print_nodeaddr(iph->saddr());
        ...
        struct hdr_mac802_15_4* dh = HDR_MAC802_15_4(p);
...
/* ======================================================================
   ...
   ====================================================================== */
void
Mac802_15_4::trace_pkt(Packet *p) 
{
	struct hdr_cmn *ch = HDR_CMN(p);
	struct hdr_mac802_15_4* dh = HDR_MAC802_15_4(p);
...
/* ======================================================================
   ...
   ====================================================================== */
inline int
Mac802_15_4::hdr_dst(char* hdr, int dst )
{
	struct hdr_mac802_15_4 *dh = (struct hdr_mac802_15_4*) hdr;
...
inline int 
Mac802_15_4::hdr_src(char* hdr, int src )
{
	struct hdr_mac802_15_4 *dh = (struct hdr_mac802_15_4*) hdr;
        if(src > -2)
...
inline int 
Mac802_15_4::hdr_type(char* hdr, u_int16_t type)
{
	struct hdr_mac802_15_4 *dh = (struct hdr_mac802_15_4*) hdr;
	if(type)
...
void
Mac802_15_4::discard(Packet *p, const char* why)
{
	hdr_mac802_15_4* mh = HDR_MAC802_15_4(p);
...
void Mac802_15_4::capture(Packet *p)
{
	/*
...
void Mac802_15_4::collision(Packet *p)
{
	switch(rx_state_) {
	case MAC_RECV:
		setRxState(MAC_COLL);
...
void Mac802_15_4::tx_resume()
{
	...
	if(pktCTRL_) {
		...
		if (mhBackoff_.busy() == 0) {
			...
			struct hdr_mac802_15_4 *mh = HDR_MAC802_15_4(pktTx_);
...
void
Mac802_15_4::recvHandler()
{
	recv_timer();
}

void
Mac802_15_4::sendHandler()
{
	send_timer();
}
...
/* ======================================================================
   ...
   ====================================================================== */
int
Mac802_15_4::check_pktCTRL()
{
	struct hdr_mac802_15_4 *mh;
	double timeout;
	...
	case MAC_Subtype_CTS:
		...
		break;
		/*
		 * IEEE 802.15.4 specs, section 9.2.8
...
void
Mac802_15_4::sendCTS(int dst, double rts_duration)
{
	Packet *p = Packet::alloc();
	hdr_cmn* ch = HDR_CMN(p);
...
// Bug reported by Mayur - Handoff triggered in Ad-hoc mode too. Bug resolved by checking if it is infrastructure mode before handoff trigger.
	...
	if(*rcount >= thresh) {
		...
		*rcount = 0;
		...
		struct hdr_mac802_15_4 *dh;
		dh = HDR_MAC802_15_4(pktTx_);
		dh->dh_fc.fc_retry = 1;
...
void
Mac802_15_4::RetransmitPROBEREP()
{
double rTime;
...
void
Mac802_15_4::RetransmitAUTHENTICATE()
{
double rTime;
...
void
Mac802_15_4::RetransmitASSOCREP()
{
double rTime;
...
/* ======================================================================
   ...
   ====================================================================== */
void
Mac802_15_4::send(Packet *p, Handler *h)
{
	int STT_; 
	double rTime;
	int Rn;
	struct hdr_mac802_15_4* dh = HDR_MAC802_15_4(p);
	struct hdr_wsn_rbc *hdr = HDR_wsn_RBC(p);	// **************** ADDED *************************
	...
	callback_ = h;
	//Rn = Random::random() % 100; set_nav(Rn * 1000);
	STT_ = hdr->MySlot;
	STT_ = STT_ * cw_ * 0.000009;
	sendDATA(p);
...
//	if(mhBackoff_.busy() == 0) {// Original Code
	if(mhDS-CSMA_Backoff_.busy() == 0) { // Modified Code
		if(is_idle()) {
			if (mhDefer_.busy() == 0) {
				/*
				 * If we are already deferring, there is no
				 ...
				 */
				if (bugFix_timer_) {
					// mhBackoff_.start(STT_, cw_, is_idle(), // Original Code
					//		  phymib_.getDIFS());
					 mhDS-CSMA_Backoff_.start(STT_, cw_, is_idle(), // Modified Code
							  phymib_.getDIFS());
			 ...
			 * the backoff timer.
			 */
			mhBackoff_.start(cw_, is_idle()); // Original Code
			//mhDS-CSMA_Backoff_.start(cw_, is_idle()); // Modified Code
			printf("Bqckoff.start\n");
		}
	}
...
void
Mac802_15_4::recv(Packet *p, Handler *h)
{
	...
	if(rx_state_ == MAC_IDLE) {
		...
		if(pktRx_->txinfo_.RxPr / p->txinfo_.RxPr >= p->txinfo_.CPThresh) {
			...
			printf("P=%6.3f C=%6.3f",pktRx_->txinfo_.RxPr / p->txinfo_.RxPr,p->txinfo_.CPThresh);
			capture(p);
		} else {
			collision(p);
		}
...
void
Mac802_15_4::recv_timer()
{
	u_int32_t src; 
	hdr_cmn *ch = HDR_CMN(pktRx_);
	hdr_mac802_15_4 *mh = HDR_MAC802_15_4(pktRx_);
	...
	/*
	 * IEEE 802.15.4 specs, section 9.2.5.6
 ...
 */
double
Mac802_15_4::txtime(double psz, double drt)
{
	...
	/* Changed by Debojyoti Dutta. This upper loop of if{}else was 
	   ...
	   Changed on 19th Oct'2000 */
        ...
        if(dst != MAC_BROADCAST) {
			...
			static int count = 0;
			if (++count <= 10) {
				printf ("MAC_802_15_4: accessing MAC cache_ array out of range (src %u, dst %u, size %d)!\n", src, dst, cache_node_count_);
				if (count == 10)
...
*/
void Mac802_15_4::update_client_table(int num, int auth_status, int assoc_status) {
...
int Mac802_15_4::find_client(int num) {
	std::list<client_table>::iterator it;
...
*/
void
Mac802_15_4::sendBEACON(int src)
{
	Packet *p = Packet::alloc();
...
void
Mac802_15_4::recvBEACON(Packet *p)
{
	struct beacon_frame *bf = (struct beacon_frame*)p->access(hdr_mac::offset_);
...
void
Mac802_15_4::sendASSOCREQ(int dst)
{
	Packet *p = Packet::alloc();
	hdr_cmn* ch = HDR_CMN(p);
...
{	
	...
	if(pktASSOCREQ_ == 0)
 		return -1;
	mh = HDR_MAC802_15_4(pktASSOCREQ_);
...
void
Mac802_15_4::sendASSOCREP(int dst)
{
	Packet *p = Packet::alloc();
	hdr_cmn* ch = HDR_CMN(p);
...
Mac802_15_4::check_pktASSOCREP()
{
 	...
 	if(pktASSOCREP_ == 0)
  		return -1;
	mh = HDR_MAC802_15_4(pktASSOCREP_);
  	switch(mh->dh_fc.fc_subtype) {
 	case MAC_Subtype_AssocRep:
 		if(!is_idle()) {
 			inc_cw();
...
void
Mac802_15_4::recvASSOCREQ(Packet *p)
{
	struct assocreq_frame *acrqf = (struct assocreq_frame*)p->access(hdr_mac::offset_);
...
void
Mac802_15_4::recvASSOCREP(Packet *p)
{
	struct assocrep_frame *acrpf = (struct assocrep_frame*)p->access(hdr_mac::offset_);
...
void
Mac802_15_4::sendAUTHENTICATE(int dst)
{
	Packet *p = Packet::alloc();
	hdr_cmn* ch = HDR_CMN(p);
...
int
Mac802_15_4::check_pktAUTHENTICATE()
{	
	struct hdr_mac802_15_4 *mh;
	double timeout;
	...
	if(pktAUTHENTICATE_ == 0)
 		return -1;
	mh = HDR_MAC802_15_4(pktAUTHENTICATE_);
...
void
Mac802_15_4::recvAUTHENTICATE(Packet *p)
{
	struct auth_frame *authf = (struct auth_frame*)p->access(hdr_mac::offset_);
...
void
Mac802_15_4::sendPROBEREQ(int dst)
{
	Packet *p = Packet::alloc();
	hdr_cmn* ch = HDR_CMN(p);
...
{	
	...
	if(pktPROBEREQ_ == 0)
 		return -1;
	mh = HDR_MAC802_15_4(pktPROBEREQ_);
...
void
Mac802_15_4::sendPROBEREP(int dst)
{
	Packet *p = Packet::alloc();
...
{	
	...
	if(pktPROBEREP_ == 0)
 		return -1;
	mh = HDR_MAC802_15_4(pktPROBEREP_);
...
void
Mac802_15_4::recvPROBEREQ(Packet *p)
{
	struct probereq_frame *prrqf = (struct probereq_frame*)p->access(hdr_mac::offset_);
...
void
Mac802_15_4::recvPROBEREP(Packet *p)
{
	struct proberep_frame *prrpf = (struct proberep_frame*)p->access(hdr_mac::offset_);
...
void Mac802_15_4::checkAssocAuthStatus() {
	if ( addr() != bss_id_ ) {
		if (authenticated == 0 && associated == 0) {
...
*/
void Mac802_15_4::update_ap_table(int num, double power) {
...
int Mac802_15_4::find_ap(int num, double power) {
...
void Mac802_15_4::deletelist() {
	ap_list1.clear();
	
}


void Mac802_15_4::add_priority_queue(int num)
{
	if (queue_head == NULL) {
...
void Mac802_15_4::push_priority(int num) {
...
void Mac802_15_4::delete_lastnode() {
	struct priority_queue *temp;
	temp = queue_head;
	if (queue_head == NULL) {
		return;
...
void Mac802_15_4::shift_priority_queue()
{
	struct priority_queue *temp;
